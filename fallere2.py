...
#these two should be Received from another divice.
#below things are just for test
realData="forteskey"
phantomData=00000001
movieName="ForTes"
...
if os.path.exists(EnvDictionary) is False:
    with open(EnvDictionary,'ab') as tempTf:
        ...
        writer.writerows([[realData,movieName],["After","After"]])
...
def MenuIngListBox(event):
    ...
    global realData
    if obj.GetStringSelection() != u"" and obj.GetStringSelection() != u'' and obj.GetStringSelection() is not None:
        realData=obj.GetStringSelection()
    print "Chk(realData at menu): "
    print realData
    #日本語化の暁には以下。
    #realData=obj.GetClientData(obj.Getselection())


    image = wx.Image("picts/"+dictRM.get(realData,"NA")+".png")
    gazou=wx.StaticBitmap(self, -1, image.ConvertToBitmap())
    
    wx.SizerItem.Replace(self,olditem, item)
...
class CenterPanel(wx.Panel):
    def __init__(self,parent):
        ...
        global gazou
        #初期画面
        """if realData is None or realData==u'' or realData==u"":
    ...
    def OnPaint(self, event=None):
        dcP = wx.PaintDC(self)
        dcP.DrawBitmap(self.bitmap, 0, 0, True)
